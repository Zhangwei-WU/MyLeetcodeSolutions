...
namespace LeetCode.P378.S2
{
    ...
    public class Solution
    {
        ...
        {
            var n = matrix.GetLength(0);
            var heap = new Heap<KeyValuePair<int, int>>(n, (a, b) => matrix[a.Key, a.Value] - matrix[b.Key, b.Value]);
            for (var i = 0; i < n; i++) heap.Add(new KeyValuePair<int, int>(i, 0));

            for (var i = 1; i < k; i++)
            {
                var peek = heap.Peek();
                if (peek.Value == n - 1) heap.Remove();
                else heap.Replace(new KeyValuePair<int, int>(peek.Key, peek.Value + 1));
            }

            return matrix[heap.Peek().Key, heap.Peek().Value];
        }
